...
command -v genisoimage >/dev/null 2>&1 || apt install genisoimage ;
...
if [[ -z $BASEISO || -z $KS || -z $KSIPADDRESS || -z $KSNETMASK || -z $KSGATEWAY || -z $KSHOSTNAME || -z $KSVLAN || -z $KSNAMESERVER ]]; then
 ...
 echo '                            -a 192.168.86.130 -m 255.255.255.0 -g 192.168.86.1 -n esx1.alphanetworks.lab -v 0 -d 192.168.86.1'
 echo 'Options:'
